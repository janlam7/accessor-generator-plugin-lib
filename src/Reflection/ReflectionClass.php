    ...
    public function getProperties(): array
    {
        ...
        if (null === $this->properties) {
            ...
            while ($vis_loc = $tokens->scan($vis_loc, [T_PRIVATE, T_PROTECTED, T_PUBLIC, T_VAR])) {
                ...
                $attributes = $this->parseAttributes($vis_loc);
//                var_dump($name);
//                var_dump(sizeof($this->properties));
    ...
    private function parseAttributes($loc): array
    {
        ...
        // Look back from T_PUBLIC, T_PROTECTED, T_PRIVATE or T_CLASS for the ']' token, indicating attributes
        $loc = $tokens->previous($loc, [T_WHITESPACE, T_STATIC, T_FINAL]);
        $attributes = [];
        while (']' === $tokens->value($loc)) {
            # Found possible end of attribute
            $start = $tokens->scanBack($loc, [T_ATTRIBUTE]);
            $attribute_text = '';
            for ($teller = $start; $teller <= $loc; $teller++) {
                $attribute_text = $attribute_text . $tokens->value($teller);
            }
            $attributes[] = $attribute_text;
            $loc = $tokens->previous($start, [T_WHITESPACE, T_STATIC, T_FINAL]); // Try to find the next ]
        }
